...
`type` char(1) NOT NULL DEFAULT 1 COMMENT '产品类型 1正常2加急3特急',

`cost` decimal(10,2) NOT NULL COMMENT '支出成本',

`country_id` smallint(4) NOT NULL,

`order_type` char(1) NOT NULL DEFAULT 1 COMMENT '订单类型-1网店，2直客，3同业',
...
ALTER TABLE `yii2_product_combo` ADD CONSTRAINT `ppid` FOREIGN KEY (`pid`) REFERENCES `yii2_product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `yii2_product_combo` ADD CONSTRAINT `ccid` FOREIGN KEY (`cid`) REFERENCES `yii2_combo` (`combo_id`) ON DELETE CASCADE ON UPDATE CASCADE;
